...
\graphicspath{{./}{docs/}{docs/phase2_eda/figures/}{phase2_eda/figures/}}

\newcommand{\figureplaceholder}[3]{%
\begin{figure}[H]
\centering
\IfFileExists{#1}{\includegraphics[width=0.92\linewidth]{#1}}{\fbox{\begin{minipage}[c][0.22\textheight][c]{0.88\linewidth}\centering Figure file not found: \texttt{\detokenize{#1}}\end{minipage}}}
\caption{#2}
\label{#3}
\end{figure}
}

\title{\textbf{변동성 기반 디지털자산 공모펀드 조성 시스템 연구 및 사업화 전략}\\[0.5em]\large 고빈도 변동성 행렬 기반 모델 포트폴리오와 B2B 운용 인프라 구현}
\author{}
\date{}
...
\end{abstract}
\noindent\textbf{Keywords:} Digital Asset, Public Fund, Volatility Matrix, PRVM, EWMA, Minimum Variance Portfolio, Crypto Portfolio Infrastructure, B2B SaaS

\section{Introduction: 문제 제기}
...
\begin{itemize}
\item 고빈도 디지털자산 데이터를 활용해 안정적인 변동성 행렬을 추정할 수 있는가?
\item 변동성 예측 기반 최소분산 포트폴리오가 동일 50개 자산 universe의 equal-weight 전략 대비 위험을 줄일 수 있는가?
\item 단일종목 cap과 monthly hold-window 구조를 적용했을 때 상품화 가능한 포트폴리오 구조를 만들 수 있는가?
\item 해당 시스템을 직접 운용 펀드가 아니라 B2B 인프라 형태로 사업화할 수 있는가?
\end{itemize}
...
\subsection{디지털자산 투자상품 및 공모형 상품 연구}
...
\subsection{실현변동성 및 변동성 행렬 추정 연구}
...
\begin{enumerate}
\item 1분봉 가격 데이터에서 일별 jump-adjusted PRVM을 계산한다.
\item Matrix-valued EWMA로 다음 날 covariance matrix를 예측한다.
\item 예측 covariance와 lagged jump volatility를 이용하여 minimum variance portfolio를 산출한다.
\item 동일 universe equal-weight 및 BTC HODL과 비교한다.
\end{enumerate}
...
본 연구는 일별 PRVM 행렬 전체에 EWMA를 적용한다. 단일 자산 변동성이 아니라 50개 자산의 $50 \times 50$ covariance matrix를 재귀적으로 갱신하기 때문에, 자산 간 상관구조의 시간 변화를 함께 반영할 수 있다.

\subsection{포트폴리오 최적화 연구}
...
\midrule
\endhead
데이터 수집 & Upbit KRW 마켓 1분봉 수집 및 후보 종목 필터링 & \texttt{\detokenize{src/phase1_data/}} \\
변동성 행렬 추정 & 일별 PRVM, raw PRVM, jump volatility 계산 & \texttt{\detokenize{src/phase3_prvm/}} \\
변동성 예측 & Matrix-valued EWMA forecast & \texttt{\detokenize{src/phase4_ewma/}} \\
포트폴리오 최적화 & Long-only minimum variance, cap 적용 & \texttt{\detokenize{src/phase5_portfolio/}} \\
백테스트 & BTC, equal-weight, minimum variance 비교 & \texttt{\detokenize{src/phase6_backtest/}} \\
대시보드 & 성과, 주문, 비중, 월별 결과 시각화 & \texttt{\detokenize{dashboard/}} \\
\bottomrule
\end{longtable}
\end{center}
...
\midrule
\endhead
Upbit KRW 전체 마켓 & 254 & \texttt{\detokenize{api.upbit.com/v1/market/all}} 기준 \\
스테이블코인 제외 후 & 249 & USD1, USDC, USDE, USDT, USDS 제외 \\
14개월 데이터 보유 후보 & 148 & 2025-02 \textasciitilde{} 2026-03 \\
...
\subsection{데이터 주기 선택의 근거: 1분봉 사용 이유}

본 연구는 변동성 행렬 추정의 입력 데이터로 1분봉 가격 데이터를 사용한다. 그 이유는 세 가지이다.

첫째, 1분 단위는 \emph{synchronization unit}으로서 대부분의 자산에서 실제로 다중 체결을 포함한다. 이를 직접 검증하기 위해 50개 universe 중 시가총액 분포를 고려해 대형주 2종(ETH, DOGE), 중형주 1종(ATOM), 소형주·롱테일 2종(SAHARA, 1INCH)을 무작위 추출하고, 각 자산별 임의 일자의 KST 09:00 기준 24시간(총 1,440분) 틱 거래 데이터를 집계하였다. 결과는 다음과 같다.

\begin{center}
\scriptsize
\begin{longtable}{@{}lp{0.14\linewidth}p{0.14\linewidth}p{0.14\linewidth}p{0.14\linewidth}p{0.14\linewidth}p{0.14\linewidth}@{}}
\toprule
...
셋째, 5분 또는 10분 단위는 microstructure noise를 완화하는 데 도움이 되지만, 변동성 행렬 추정 단계에서는 정보 손실이 크다. 대형 자산의 분당 38\textasciitilde{}54회 체결 정보를 5\textasciitilde{}10배 단위로 축약하면 jump, volatility clustering, 자산 간 공분산 변화처럼 본 연구의 핵심 추정 대상이 묽어진다. 또한 PRVM의 pre-averaging 구조는 1분 단위 수익률에서 발생할 수 있는 잔여 noise를 추가로 완화하도록 설계되어 있어, 1분봉은 관측 빈도와 추정 안정성 사이의 적절한 절충점이 된다. 이는 FIVAR(Shin et al. 2025)의 pre-averaging 권고 frequency와도 부합하며, 본 연구는 이상의 근거로 \textbf{m = 1,440 ($24h \times 60min$)} 을 base sampling grid로 채택한다.

포트폴리오 성과 평가는 실험 설계에 맞추어 10분 수익률 기준으로 수행하였다.

\subsection{최종 포트폴리오 Universe}

최종 universe는 평균 거래대금 기준 상위 50개 디지털자산이다. 상위 12개 자산은 다음과 같다.

\begin{center}
\scriptsize
\begin{longtable}{@{}p{0.20\linewidth}p{0.20\linewidth}p{0.20\linewidth}p{0.20\linewidth}p{0.20\linewidth}@{}}
\toprule
...
\subsection{디지털자산 가격 데이터의 기초 EDA}
...
보고서용 EDA는 \texttt{\detokenize{price_panel.csv}}의 \texttt{\detokenize{trading_day}} 기준으로 2025-03-01부터 2026-03-30까지 사용하였다. 원천 패널은 2026-03-31 23:59까지 존재하지만, \texttt{\detokenize{trading_day = 2026-03-31}}은 900개 분봉만 포함된 partial day이므로 보고서용 그림과 요약 통계에서는 제외하였다. 그 결과 50개 자산 모두에서 568,800개의 1분 로그수익률과 395개의 일별 로그수익률 관측치를 확보하였다.

첫 번째 EDA는 대표 자산의 1분 로그수익률 분포이다. BTC, ETH, XRP, SOL처럼 거래대금이 크고 시장 대표성이 높은 자산을 선정하여 histogram을 그리고, 동일 평균과 표준편차를 갖는 정규분포 밀도곡선을 함께 표시하였다. 극단값으로 인해 중심부가 과도하게 눌려 보이는 문제를 피하기 위해 Figure 5의 x축은 각 자산별 1\%\textasciitilde{}99\% 분위수 구간으로 제한하였으며, 정규분포 곡선은 전체 표본의 평균과 표준편차를 사용하였다.

\figureplaceholder{phase2_eda/figures/fig05_log_return_histograms.png}{Figure 5. Representative digital asset log-return distributions}{fig:figure-5-representative-digital-asset-log-return-distributio}

대표 자산의 1분 수익률 표준편차는 BTC 6.04 bps, ETH 9.85 bps, XRP 10.81 bps, SOL 11.43 bps로 나타났다. 같은 기간 BTC의 1분 수익률 범위는 -1.79\%\textasciitilde{}1.65\%였으나, XRP는 -7.21\%\textasciitilde{}11.99\%까지 관측되어 개별 자산의 tail event 규모가 크게 다르다는 점을 보여준다.
...
\figureplaceholder{phase2_eda/figures/fig06_asset_mean_return_bar.png}{Figure 6. Mean 1-minute log return by asset}{fig:figure-6-mean-1-minute-log-return-by-asset}

세 번째 EDA는 log ordinary kurtosis box plot이다. 기존 Phase 2 리포트의 \texttt{\detokenize{kurtosis_1m}}은 excess kurtosis 기준이므로, 본 보고서용 그림에서는 \texttt{\detokenize{ordinary kurtosis = excess kurtosis + 3}}으로 변환한 뒤 log를 취했다. Student-$t$ distribution with degrees of freedom 5의 ordinary kurtosis는 9이므로, 기준선은 \texttt{\detokenize{log(9) = 2.197}}로 설정하였다.

\figureplaceholder{phase2_eda/figures/fig07_log_kurtosis_boxplot.png}{Figure 7. Log ordinary kurtosis box plot}{fig:figure-7-log-ordinary-kurtosis-box-plot}
...
원천 데이터는 Upbit의 월별 1분봉 CSV이다. 각 CSV에는 \texttt{\detokenize{date_time_utc}}, \texttt{\detokenize{open}}, \texttt{\detokenize{high}}, \texttt{\detokenize{low}}, \texttt{\detokenize{close}}, \texttt{\detokenize{acc_trade_price}}, \texttt{\detokenize{acc_trade_volume}}이 포함된다. 전처리 과정에서는 UTC timestamp를 KST로 변환하고, 50개 최종 자산에 대해 close price wide panel을 구성한다. 결측치는 forward fill 후 backward fill로 처리되었으며, 최종 가격 패널에는 결측치가 없도록 정리되었다.
...
\subsection{PRVM 기반 변동성 행렬 추정}

일별 PRVM은 KST 09:00 기준 하루에 해당하는 1분 로그수익률 블록을 입력으로 계산한다. 하루는 24시간이므로 $m=1440$개의 1분 수익률을 갖는다. Pre-averaging window는 $K=\lfloor\sqrt{1440}\rfloor=37$로 설정하였다.
...
계산 절차에서는 raw PRVM, jump-adjusted PRVM, jump volatility matrix를 모두 산출한다. PRVM 산출 후에는 eigenvalue floor를 $1\times 10^{-10}$으로 설정한 PSD projection을 적용한다. 이는 covariance matrix가 수치적으로 양의 준정부호가 되도록 보장하고, 이후 QLIKE 계산에서 log determinant가 발산하지 않도록 하기 위한 처리이다.
...
\endhead
PRVM 일수 & 423 \\
기간 & 2025-02-01 \textasciitilde{} 2026-03-30 \\
자산 수 & 50 \\
PRVM shape & 423 x $50 \times 50$ \\
PSD floor & 1e-10 \\
...
EWMA의 입력인 PRVM은 PSD projection을 거친 대칭 행렬이며, rolling EWMA forecast는 PSD 행렬들의 정규화된 가중평균이다. 따라서 이론적으로는 PSD 성질이 보존된다. 다만 수치 계산 과정에서 비대칭 오차나 아주 작은 음의 eigenvalue가 발생할 수 있으므로, 각 rolling forecast 이후에 다시 PSD projection을 적용한다. PSD projection은 (1) $(M+M^\top)/2$로 행렬을 대칭화하고, (2) eigenvalue를 $1\times 10^{-10}$ 이상으로 floor한 뒤, (3) 재구성된 행렬을 다시 대칭화하는 절차이다. 따라서 symmetry error가 0으로 확인되는 것은 EWMA 구조뿐 아니라 이 PSD projection 절차의 결과이기도 하다.
...
\endhead
forecast days & 395 \\
target period & 2025-03-01 \textasciitilde{} 2026-03-30 \\
forecast shape & 395 x $50 \times 50$ \\
mean MSPE (x10\textasciicircum{}3) & 4.3239 \\
mean QLIKE (x10\textasciicircum{}-2) & -2.8708 \\
...
\subsection{Concentration Risk 완화}
...
active count range & 8 \textasciitilde{} 20 \\
...
\midrule
\endhead
실험 기간 & 2025-03-01 \textasciitilde{} 2026-03-30 \\
...
\begin{itemize}
\item Minimum variance portfolio: EWMA covariance와 jump volatility를 반영한 25\% cap long-only portfolio
\item Equal-weight portfolio: 동일 50개 자산에 균등 비중
\item BTC HODL: BTC 100\% buy-and-hold
\end{itemize}
...
\toprule
Model & Input Matrix & Target Matrix & Forecast Days & Mean MSPE (x10\textasciicircum{}3) & Mean QLIKE (x10\textasciicircum{}-2) \\
\midrule
\endfirsthead
\toprule
Model & Input Matrix & Target Matrix & Forecast Days & Mean MSPE (x10\textasciicircum{}3) & Mean QLIKE (x10\textasciicircum{}-2) \\
...
비교 결과, jump-adjusted PRVM EWMA는 raw PRVM EWMA보다 평균 MSPE가 낮았다. \texttt{\detokenize{MSPE (x10^3)}} 기준 raw-minus-adjusted 차이는 0.1761이며, 이는 raw PRVM 대비 약 3.91\% 낮은 MSPE에 해당한다. 또한 395개 forecast day 중 308일, 즉 약 77.97\%의 날짜에서 jump-adjusted PRVM EWMA의 MSPE가 더 낮았다.

QLIKE 기준으로도 jump-adjusted PRVM EWMA가 더 낮은 손실을 보였다. \texttt{\detokenize{QLIKE (x10^-2)}} 기준 평균값은 jump-adjusted 방식이 -2.8708, raw 방식이 -2.8672로 나타났으며, raw-minus-adjusted 기준 차이는 0.0036이다. 395일 중 297일, 즉 약 75.19\%의 날짜에서 jump-adjusted PRVM EWMA의 QLIKE가 더 낮았다.
...
\begin{center}
\scriptsize
\begin{longtable}{@{}lp{0.10\linewidth}p{0.09\linewidth}p{0.11\linewidth}p{0.12\linewidth}p{0.10\linewidth}p{0.10\linewidth}p{0.10\linewidth}p{0.11\linewidth}@{}}
\toprule
...
\subsection{Monthly Hold-Window Backtest}
...
\section{사업화 및 비즈니스모델 구현}

\subsection{사업화 문제 정의}
...
\subsection{BlackRock Aladdin 전략 벤치마킹}
...
\subsection{본 프로젝트의 BM Strategy}
...
\begin{enumerate}
\item 고객사가 대상 universe 또는 보유 포트폴리오 데이터를 시스템에 입력한다.
\item 시스템은 고빈도 가격 데이터와 변동성 행렬 모델을 기반으로 위험 구조를 분석한다.
\item 시스템은 목표 포트폴리오 비중과 리밸런싱 방향을 대시보드 또는 API로 제공한다.
\item 고객사는 자체 내부통제와 운용 절차에 따라 실제 거래를 집행한다.
\end{enumerate}
...
\subsection{규제 환경 현황 및 리스크 포지셔닝}

2026년 5월 24일 기준 가상자산 관련 법률·제도 환경은 B2B 인프라 라이선싱 모델 관점에서 검토할 필요가 있다. 핵심 전제는 \texttt{\detokenize{No Custody / B2B Only}}이다. 즉, 본 프로젝트는 고객 자산을 보관하지 않고, 개인투자자에게 직접 판매하지 않으며, 고객사의 상품 개발과 운용 판단에 필요한 정량 분석 결과를 제공하는 인프라로 포지셔닝한다.
...
\begin{itemize}
\item 투자자문업 해당 여부
\item 가상자산사업자 신고 필요 여부
\item 고객 자산 custody 여부
\item 리밸런싱 signal 제공이 투자권유 또는 자문에 해당하는지
\item 공모형 상품 출시 시 자본시장법 및 가상자산 관련 법률 적용 여부
\item B2B 표준계약서, 면책 조항, 고객 책임 범위 설계
\end{itemize}

% [Table 권장] Table 8. 가상자산 관련 법률·제도 현황과 프로젝트 대응 전략. 위 표를 최종본에 포함하되, 시행일과 수치는 금융위 등 공식 출처로 재확인한다.


\subsection{규제 마일스톤 기반 시장 진입 액션플랜}
...
\midrule
\endhead
Phase 1. 시스템 완성 및 시장 진입 준비 & 2026 H1\textasciitilde{}H2 & 기술 검증과 법적 구조 준비 & 백테스팅 완료, 성과 리포트 작성, IP 보호, 영업비밀 관리, B2B 표준계약서, 금융규제 샌드박스 컨설팅 신청 \\
Phase 2. 규제 환경 대응 및 파일럿 계약 & 2026 H2\textasciitilde{}2027 H1 & 제한적 출시와 PoC 확보 & 샌드박스 정기신청, 크립토 헤지펀드·GTM 전략사 파이프라인 구축, LOI 확보, 파일럿 대시보드 제공 \\
Phase 3. STO 시장 연계 및 제도권 영업 & 2027 H1\textasciitilde{}H2 & 자산운용사·증권사 대상 영업 & STO 제도 시행 전후 토큰화 상품·디지털자산 상품 개발 니즈 선점, 자산운용사 파일럿 계약 \\
Phase 4. 현물 ETF·법인 투자 허용 대비 확장 & 2027\textasciitilde{}2028 이후 & 대형 기관 고객 확장 & 법인 투자 및 현물 ETF 허용 시 대형 운용사, 증권사, 기관 트레저리 대상 API·대시보드 확장 \\
\bottomrule
\end{longtable}
\end{center}
...
\begin{enumerate}
\item 백테스팅 및 성과 검증을 완성하여 외부 고객에게 제시 가능한 성과 리포트를 작성한다.
\item 핵심 알고리즘과 데이터 파이프라인에 대해 영업비밀 관리 및 IP 보호 체계를 마련한다.
\item B2B 표준계약서, 책임 제한, 투자자문업 경계 문구를 포함한 법적 구조를 설계한다.
\item 금융규제 샌드박스 컨설팅을 신청하여 현재 서비스 구조가 혁신금융서비스로 검토 가능한지 확인한다.
\item STO 제도 시행 전 자산운용사·증권사 대상 데모 영업을 시작하고, 크립토 헤지펀드 및 GTM 전략사를 대상으로 PoC를 확보한다.
\end{enumerate}
...
프로토타입은 \texttt{\detokenize{dashboard/index.html}}, \texttt{\detokenize{dashboard/styles.css}}, \texttt{\detokenize{dashboard/app.js}}, \texttt{\detokenize{dashboard/data/dashboard_snapshots.json}}으로 구성된 정적 웹 대시보드이다. 대시보드 데이터는 monthly cap25 포트폴리오 비중과 백테스트 결과를 기반으로 생성되었다.
...
\begin{itemize}
\item Demo date slider: 특정 날짜의 포트폴리오 상태 확인
\item 7D / 14D cycle selector: hold window 선택
\item Simple / Managed mode selector: 운용 방식 선택
\item Entry AUM input and Current AUM readout: 매월 상품 시작 원금과 날짜별 평가금액을 분리하여 표시
\item KPI cards: total return, 전체 active hold-window Sharpe, MDD, annualized volatility, active assets
\item Performance chart: minimum variance, equal-weight, BTC 비교
\item Drawdown chart: 변동성 자체가 아니라 월간 상품의 고점 대비 낙폭 경로 확인
\item Monthly return chart: 과거 월별 hold-window return 비교
\item Orders table: current, target, delta weight 및 주문 금액을 상단에 표시
\item Portfolio donut chart: 목표 또는 현재 포트폴리오 비중을 상위 종목과 Other bucket으로 표시
\end{itemize}
...
\subsection{대시보드의 사업적 의미}

대시보드는 본 프로젝트의 Aladdin-style BM을 구체화한다. 고객은 내부 알고리즘을 직접 보지 않고, 분석 결과와 리밸런싱 지시만 확인한다. 이는 IP 보호와 고객 사용성 측면에서 모두 중요하다. 또한 Entry AUM input, Current AUM readout, order table, portfolio donut chart는 단순 성과 시각화를 넘어 실제 운용 업무에 가까운 기능을 제공한다.

향후 대시보드는 다음 방향으로 확장될 수 있다.

\begin{itemize}
\item 고객별 universe 업로드
\item cap, turnover, liquidity constraint 설정
\item 거래비용 시뮬레이션
\item API key 기반 기관 고객 접근
\item 투자위원회용 PDF 리포트 자동 생성
\item real-time data ingestion
\end{itemize}


\section{인터뷰 기반 사업화 가능성 검증}

\subsection{인터뷰 목적 및 설계}
...
\subsection{인터뷰 기반 Future Works}
...
\subsection{규제 및 사업 검토}

실제 서비스 출시 전에는 법률 검토가 필수적이다. 특히 다음 쟁점을 검토해야 한다.

\begin{itemize}
\item 모델 포트폴리오 제공이 투자자문에 해당하는지
\item 리밸런싱 주문 방향 제공이 투자권유 또는 일임 운용으로 해석될 가능성
\item 고객 자산을 보관하지 않는 no custody 구조의 규제상 의미
\item 공모형 상품 출시 시 자본시장법과 가상자산 관련 법률의 적용 관계
\item 디지털자산기본법 제정 이후 사업자 유형 분류 가능성
\end{itemize}
...
\bibitem{ref1} Shin, M., Kim, D., Wang, Y., \& Fan, J. (2025). \emph{Factor and Idiosyncratic VAR Volatility Matrix Models for Heavy-Tailed High-Frequency Financial Observations}.
\bibitem{ref2} Jacod, J., Li, Y., Mykland, P. A., Podolskij, M., \& Vetter, M. (2009). Microstructure noise in the continuous case: The pre-averaging approach.
\bibitem{ref3} Christensen, K., Kinnebrock, S., \& Podolskij, M. (2010). Pre-averaging estimators of the ex-post covariance matrix.
\bibitem{ref4} Ait-Sahalia, Y., \& Xiu, D. (2016, 2017). High-frequency covariance and volatility estimation.
\bibitem{ref5} RiskMetrics Group. (1996). \emph{RiskMetrics Technical Document}.
\bibitem{ref6} 금융위원회. 제2차 가상자산위원회 개최: 가상자산이용자보호법 2단계 입법 논의 착수. \url{https://www.fsc.go.kr/no010101/83855}
\bibitem{ref7} 금융위원회. 민·관 합동 토큰증권 협의체 Kick-off 회의 개최: 토큰증권 제도화 법 2027.2.4 시행 예정. \url{https://www.fsc.go.kr/no010101/86371}
\bibitem{ref8} 금융위원회. 디지털자산기본법 정부 검토안 관련 가상자산위원회 논의 자료. \url{https://www.fsc.go.kr/po010106/86373}
\bibitem{ref9} [추가 필요] 한국 디지털자산 현물 거래량 비중 관련 신뢰 가능한 시장 데이터 출처. Kaiko, CCData, 국내 증권사 리포트, 금융당국 자료 중 선택.
\end{thebibliography}
\appendix

\subsection{최종 선정 50개 종목 목록}


\subsection{자산별 평균 거래 빈도 및 1분봉 선택 근거}

본 부록은 \S{}3.4 "1분봉 사용 이유"에서 제시한 틱 거래 빈도 실증치의 원본 집계 결과와 표본 선정 절차를 정리한다.

\textbf{표본 선정.} 50개 universe 중 시가총액 분포를 골고루 반영하기 위해 다음 5종을 추출하였다. 대형주 2종(ETH, DOGE), 중형주 1종(ATOM), 소형주·롱테일 2종(SAHARA, 1INCH). 각 자산에 대해 분석 기간 내 임의 일자를 한 일자씩 선택하였고, 일자는 사전에 변동성 이벤트나 거래소 공지가 없는 평이한 거래일을 기준으로 골랐다. 집계 윈도우는 KST 09:00부터 다음날 09:00까지 24시간(총 1,440분)이며, 본 연구의 trading day 정의와 일치한다.
...
\textbf{집계 합계.} 총 138,947 ticks. 다섯 표본 단순 평균은 분당 19.3 trades, 거래 발생 분 기준으로는 평균 20.5 trades.

\textbf{해석.} (1) 대형주는 1분당 평균 38\textasciitilde{}54회 체결로, 1분 close price가 다중 체결을 자연스럽게 평활화한다. (2) 중·소형주도 거래가 발생한 분(active minute) 기준으로는 평균 3회 이상이 체결되어, 1분 윈도우가 정보를 보존한다. (3) 가장 거래가 희박한 1INCH도 분당 0.61회 체결되었는데, 초 단위로 환산하면 분당 거래 발생 비중 21.5\%가 더 떨어져 sparsity 문제가 심화된다. 따라서 1분 단위는 50개 universe 전체에서 (i) 대형주의 정보 손실, (ii) 소형주의 sparsity 양 측면을 모두 통제하는 절충점이다.

\textbf{한계.} 자산별 단일 일자 추출 결과이므로 시장 변동성 국면별 빈도 차이는 별도로 분석되지 않았다. 다만 본 검증의 목적은 1분 윈도우가 universe 전체에서 동작 가능한 sampling grid임을 보이는 것이므로, 50개 자산 전체에 대한 일자별 평균 빈도 분석은 후속 과제로 둔다.

\subsection{디지털자산 로그수익률 EDA 보조자료}
...
\midrule
\endhead
Figure 5 & \texttt{\detokenize{docs/phase2_eda/figures/fig05_log_return_histograms.png}} & BTC, ETH, XRP, SOL의 1분 로그수익률 histogram 및 normal density overlay \\
Figure 6 & \texttt{\detokenize{docs/phase2_eda/figures/fig06_asset_mean_return_bar.png}} & 50개 자산의 평균 1분 로그수익률 bar plot \\
Figure 7 & \texttt{\detokenize{docs/phase2_eda/figures/fig07_log_kurtosis_boxplot.png}} & \texttt{\detokenize{log(ordinary kurtosis)}} box plot 및 Student-$t$(5) 기준선 \texttt{\detokenize{log(9)}} \\
Summary CSV & \texttt{\detokenize{data/processed/phase2_eda/report_log_return_summary.csv}} & 자산별 1분/일별 로그수익률 요약, ordinary/excess/log kurtosis \\
Preview HTML & \texttt{\detokenize{docs/phase2_eda/report_eda_preview.html}} & 위 세 그림과 summary table을 함께 확인하는 미리보기 페이지 \\
\bottomrule
\end{longtable}
\end{center}

\subsection{주요 파라미터 표}
...
\subsection{Active Hold-Window 백테스트 성과표}


\subsection{대시보드 화면 캡처}


\subsection{인터뷰 질문지 및 요약}
...
\subsection{구현 파일 구조 및 실행 스크립트 목록}
...
\midrule
\endhead
\texttt{\detokenize{scripts/run_phase3_prvm.py}} & PRVM 산출 \\
\texttt{\detokenize{scripts/run_phase4_ewma.py}} & EWMA forecast 산출 \\
\texttt{\detokenize{scripts/run_phase5_portfolio.py}} & Minimum variance portfolio 산출 \\
\texttt{\detokenize{scripts/run_phase6_backtest.py}} & 백테스트 실행 \\
\texttt{\detokenize{scripts/build_dashboard_data.py}} & 대시보드 데이터 생성 \\
\texttt{\detokenize{dashboard/}} & 정적 대시보드 구현 \\
\bottomrule
\end{longtable}
\end{center}
\end{document}
