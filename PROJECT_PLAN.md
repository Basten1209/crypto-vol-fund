...
- 하루 144 = 1440 / 10 구간
- **Dashboard 표시 원칙**: drawdown chart와 volatility KPI를 분리한다. Sharpe KPI는 선택 날짜의 짧은 partial window가 아니라 Phase 6 active hold-window 전체 성과표를 사용해 짧은 샘플의 연율화 왜곡을 피한다. AUM 입력값은 매월 상품의 시작 원금인 Entry AUM으로 고정하고, 날짜별 평가금액은 Current AUM으로 별도 표시한다. hold start는 return 0% baseline으로 표시해 진입 이전 일간 수익률이 상품 성과에 섞이지 않게 한다. 운용 의사결정 화면인 Orders와 Portfolio donut을 성과 차트보다 위에 배치한다.
