...
class FIVARModel:
    ...
    def fit(self, prvm_matrices: Dict, idio_matrices: Dict, tickers: List[str]):
        ...
        if self.model_type == 'ols':
            ...
            tasks = [(i, y_scaled[:, self.r + i].copy(), X_scaled.copy(), self.p, n_reg, c_eta_candidates, self.model_type) for i in range(self.p)]
...
# [추가] 병렬 처리를 위한 워커 함수
def process_forecast_step(t, prvm_data_all, idio_data_all, dates, tickers, gics_data, in_sample_size, num_factors, h_lag, l_window):
    """롤링 윈도우의 단일 예측 단계를 처리합니다."""
    # 1. In-sample 데이터셋 정의
    ...
    # 2. Ground truth (실현 변동성) 정의
    ground_truth_matrix = calculate_poet_prvm(prvm_data_all[t], gics_data, num_factors)
    ...
    daily_forecasts['POET-PRVM'] = calculate_poet_prvm(prvm_t_minus_1, gics_data, num_factors)
    
    # FIVAR 모델들 (OLS, LASSO, H-LASSO)
    for model_type in ['OLS', 'LASSO', 'H-LASSO']:
        ...
        daily_forecasts[model_type] = prediction
        
    return daily_forecasts, ground_truth_matrix, dates[t]
...
def main():
    ...
    # [수정] 메인 루프를 병렬 처리로 변경
    tasks = [delayed(process_forecast_step)(t, prvm_data_all, idio_data_all, dates, tickers, gics_data, in_sample_size, num_factors, h_lag, l_window) for t in range(start_idx, len(dates))]
    
    print(f"\n{len(tasks)}일 간의 롤링 윈도우 예측을 병렬로 시작합니다...")
    results = Parallel(n_jobs=-1)(tqdm(tasks, desc="전체 롤링 윈도우 예측"))
    
    # 병렬 처리 결과 정리
    forecasts = {model: [] for model in models}
    ground_truths = []
    forecast_dates = []
    for daily_forecasts_dict, ground_truth_matrix, forecast_date in results:
        for model in models:
            forecasts[model].append(daily_forecasts_dict[model])
        ground_truths.append(ground_truth_matrix)
    ...
    # --- 6. 포트폴리오 리스크 분석 (주석 처리됨) ---
    """
    # ... (포트폴리오 분석 코드는 변경 없이 주석 처리 유지) ...
    """
