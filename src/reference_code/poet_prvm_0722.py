...
def apply_poet(prvm, gics_sectors, num_factors=3):
    ...
    Returns:
        tuple: (최종 POET-PRVM 행렬, 특이 변동성 행렬, 요인 부분 행렬)
    """
    p = prvm.shape[0]
    ...
    # PSD 처리된 최종 행렬, 특이 변동성 행렬, 그리고 요인 부분 행렬을 반환
...
def calculate_mspe(forecasts, ground_truths):
    ...
    T = len(forecasts)
    if T == 0:
        print("경고: MSPE 계산을 위한 데이터가 없습니다.")
        return np.nan
...
def calculate_qlike(forecasts, ground_truths, reg=1e-8):
    ...
    T = len(forecasts)
    if T == 0:
        print("경고: QLIKE 계산을 위한 데이터가 없습니다.")
        return np.nan
...
def main():
    ...
    # 2. 모든 날짜에 대해 POET-PRVM, Idio Matrix 및 Factor Part 계산
    ...
    factor_parts_all = np.array([res[2] for res in results_list]) # 요인 부분(Factor Part)을 저장
    ...
    for period_name, (start_idx, end_idx) in periods.items():
        ...
        # --- MODIFIED ---
        # 평가 대상을 '요인 부분 행렬(factor part)'로 변경합니다.
        # 이는 상위 3개 요인에 대해서만 예측 오차를 평가하기 위함입니다.
        forecasts = factor_parts_all[start_idx-1 : end_idx]
        ground_truths = factor_parts_all[start_idx : end_idx+1]
        
        # 요인 부분 행렬을 대상으로 오차를 계산합니다.
    ...
    print("\n참고: 이제 평가는 전체 행렬이 아닌 상위 3개 요인 부분에 대해서만 수행됩니다.")
