# -*- coding: utf-8 -*-
"""
FIVAR Model Implementation and Evaluation (Final psi Correction)
...
주요 수정 사항 (v5):
- [핵심 수정] PRVM 계산 시 사용되는 상수 'psi' 값을 논문의 정확한 이론값인 '1/12'로
  최종 수정했습니다.
"""
...
class Preprocessor:
    ...
    def get_idiosyncratic_matrices(self, prvm_data: np.ndarray) -> np.ndarray:
        """전체 기간의 PRVM 데이터에 POET 기법을 병렬로 적용합니다."""
...
class POETPRVMEstimator(BaseEstimatorModel):
    ...
    def predict(self, prvm_matrix: np.ndarray, **kwargs) -> np.ndarray:
        return project_psd(self.preprocessor._apply_poet(prvm_matrix))
...
class BaseFIVAREstimator(BaseEstimatorModel):
    def __init__(self, model_name: str, num_factors: int, h_lag: int, l_window: int):
        ...
        self.p=None; self.factor_evecs=None; self.idio_evecs=None; self.factor_regressors=None; self.idio_regressors=None; self.last_known_eigenvalues=None; self.idio_mean_forecast=None; self.scaler_X=None; self.scalers_y=None; self.omega_F=None; self.omega_I=None; self.tau_F=None; self.tau_I=None
    def fit(self, prvm_train: np.ndarray, idio_train: np.ndarray):
        ...
        X_list = [np.hstack([factor_evals[i:n_train-self.h+i], idio_evals[i:n_train-self.h+i]]) for i in range(self.h)]; X_data = np.hstack(X_list)
        n_reg = len(y_data)
        ...
        self.scaler_X = StandardScaler().fit(X_truncated); X_scaled = self.scaler_X.transform(X_truncated)
        ...
        self._fit_factor_models(X_scaled, y_scaled); self._fit_idio_models(X_scaled, y_scaled, idio_evals)
        self.last_known_eigenvalues = np.hstack([factor_evals[-self.h:], idio_evals[-self.h:]])
    def predict(self) -> np.ndarray:
        if self.last_known_eigenvalues is None: raise RuntimeError("Model not fitted.")
        X_pred_raw = self.last_known_eigenvalues.flatten().reshape(1, -1); X_pred_truncated = self._apply_truncation_to_X(X_pred_raw); X_pred_scaled = self.scaler_X.transform(X_pred_truncated)
        pred_evals_scaled = np.zeros(self.p + self.r); X_pred_factor_scaled = X_pred_scaled[:, :self.r * self.h]
        for i in range(self.r): pred_evals_scaled[i] = self.factor_regressors[i].predict(X_pred_factor_scaled)[0]
        pred_factor_evals = np.array([self.scalers_y[i].inverse_transform(pred_evals_scaled[i].reshape(1, -1))[0, 0] for i in range(self.r)])
        if self.model_name == 'OLS': pred_idio_evals = self.idio_mean_forecast
        else:
            pred_idio_evals_scaled = np.zeros(self.p)
            for i in range(self.p): pred_idio_evals_scaled[i] = self.idio_regressors[i].predict(X_pred_scaled)[0]
            pred_idio_evals = np.array([self.scalers_y[self.r + i].inverse_transform(pred_idio_evals_scaled[i].reshape(1, -1))[0, 0] for i in range(self.p)])
    ...
    def _apply_truncation_to_X(self, X_data: np.ndarray) -> np.ndarray:
        ...
        return X_truncated
    def _fit_factor_models(self, X_scaled, y_scaled): raise NotImplementedError
    def _fit_idio_models(self, X_scaled, y_scaled, idio_evals): raise NotImplementedError

class FIVAR_OLS_Estimator(BaseFIVAREstimator):
    def __init__(self, num_factors: int, h_lag: int, l_window: int): super().__init__("OLS", num_factors, h_lag, l_window)
    def _fit_factor_models(self, X_scaled, y_scaled):
        self.factor_regressors = [None] * self.r; X_factor_scaled = X_scaled[:, :self.r * self.h]
        for i in range(self.r): self.factor_regressors[i] = LinearRegression(fit_intercept=False).fit(X_factor_scaled, y_scaled[:, i])
    def _fit_idio_models(self, X_scaled, y_scaled, idio_evals): self.idio_mean_forecast = np.mean(idio_evals[-self.l:], axis=0)

class FIVAR_LASSO_Estimator(BaseFIVAREstimator):
    def __init__(self, num_factors: int, h_lag: int, l_window: int): super().__init__("LASSO", num_factors, h_lag, l_window)
    def _fit_factor_models(self, X_scaled, y_scaled):
        self.factor_regressors = [None] * self.r; X_factor_scaled = X_scaled[:, :self.r * self.h]
        for i in range(self.r): self.factor_regressors[i] = LinearRegression(fit_intercept=False).fit(X_factor_scaled, y_scaled[:, i])
    def _fit_idio_models(self, X_scaled, y_scaled, idio_evals):
        self.idio_regressors = [None] * self.p; n_reg = X_scaled.shape[0]
        def find_best_lasso(i):
            y_i = y_scaled[:, self.r + i]; best_bic, best_reg = np.inf, None
            for c_eta in np.linspace(0.1, 10.0, 20):
                eta_I = c_eta * np.sqrt(np.log(self.p) / n_reg); reg = Lasso(alpha=eta_I, fit_intercept=False, max_iter=2000).fit(X_scaled, y_i)
                rss = np.sum((y_i - reg.predict(X_scaled))**2); num_params = np.sum(np.abs(reg.coef_) > 1e-6)
...
class FIVAR_HLASSO_Estimator(BaseFIVAREstimator):
    def __init__(self, num_factors: int, h_lag: int, l_window: int): super().__init__("H-LASSO", num_factors, h_lag, l_window)
    def _fit_factor_models(self, X_scaled, y_scaled):
        self.factor_regressors = [None] * self.r; X_factor_scaled = X_scaled[:, :self.r * self.h]
        for i in range(self.r): self.factor_regressors[i] = CustomHuberRegressor(alpha=0.0, epsilon=self.tau_F).fit(X_factor_scaled, y_scaled[:, i])
    def _fit_idio_models(self, X_scaled, y_scaled, idio_evals):
        self.idio_regressors = [None] * self.p; n_reg = X_scaled.shape[0]
        def find_best_hlasso(i):
            y_i = y_scaled[:, self.r + i]; best_bic, best_reg = np.inf, None
            for c_eta in np.linspace(0.1, 10.0, 20):
                eta_I = c_eta * np.sqrt(np.log(self.p) / n_reg); reg = CustomHuberRegressor(alpha=eta_I, epsilon=self.tau_I).fit(X_scaled, y_i)
                rss = np.sum((y_i - reg.predict(X_scaled))**2); num_params = np.sum(np.abs(reg.coef_) > 1e-6)
                bic = n_reg * np.log(rss / n_reg) + num_params * np.log(n_reg) if rss > 0 else np.inf
                if bic < best_bic: best_bic, best_reg = bic, reg
            return i, best_reg
