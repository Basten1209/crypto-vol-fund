            ...
            for model in models:
                ...
                for c0 in tqdm(exposure_constraints, leave=False, desc=f"Exposure {model}"):
                    ...
                    for i, day_idx in enumerate(period_forecast_indices):
                        ...
                        # --- [수정] DCP 오류 방지를 위해 최종 리스크 행렬을 PSD로 변환 ---
                        total_vol_forecast_psd = project_psd(total_vol_forecast)
                        
                        n_assets = total_vol_forecast_psd.shape[0]
                        
                        w = cp.Variable(n_assets)
                        prob = cp.Problem(cp.Minimize(cp.quad_form(w, total_vol_forecast_psd)),
