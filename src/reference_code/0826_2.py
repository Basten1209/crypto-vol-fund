# -*- coding: utf-8 -*-
"""
FIVAR Model Implementation and Evaluation (Optimized Logic)
...
주요 수정 사항 (v8):
- [개선] 포트폴리오 최적화 시, 이미 PSD로 보장된 두 행렬(연속/점프 변동성)의 합에
  중복으로 PSD 변환을 적용하던 로직을 제거하여 코드의 효율성과 명확성을 높였습니다.
"""
...
class PortfolioOptimizer:
            ...
            for model_name in self.evaluator.models:
                ...
                for c0 in tqdm(exposure_constraints, leave=False, desc=f"   Exposure (c0)"):
                    ...
                    for i, is_in_period in enumerate(period_mask):
                        ...
                        # [개선] 이미 PSD가 보장된 행렬들의 합이므로 중복 PSD 변환 제거
    ...
    def _optimize_portfolio(self, cov_matrix: np.ndarray, c0: float) -> Union[np.ndarray, None]:
        ...
        try:
            # 최적화 안정성을 위해 cov_matrix에 작은 항등행렬을 더해줌 (regularization)
