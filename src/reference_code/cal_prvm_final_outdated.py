...
def load_and_clean_data(filepath: str) -> pd.DataFrame:
    ...
    # 인덱스를 datetime 객체로 변환, 변환 실패 시 NaT로 설정
    df.index = pd.to_datetime(df.index, errors='coerce')

    # NaT 인덱스를 가진 행(유효하지 않은 타임스탬프) 제거
...
def calculate_prvm(data: pd.DataFrame, start_year: int = 2016, end_year: int = 2019) -> tuple[dict, list]:
    """
    벡터화된 고속 연산을 사용하여 Pre-averaged Realized Volatility Matrix (PRVM)를 계산합니다.
    입력 데이터는 이미 로그 변환된 가격 데이터라고 가정합니다.
    """
    print(f"PRVM 계산을 시작합니다 (고성능 벡터화 버전). 대상 기간: {start_year}-{end_year}")

    tickers = data.columns.tolist()
    num_tickers = len(tickers)

    # ★★★ 수정된 부분 ★★★
    # 입력 데이터가 이미 로그 가격이므로 np.log()를 호출하지 않습니다.
    log_prices = data
    ...
    for date, daily_log_returns in tqdm(daily_groups, desc=f"일별 PRVM 계산 중 ({start_year}-{end_year})"):
        ...
        # --- 1. Y_bar 행렬 고속 계산 (np.convolve 사용) ---
        weights_y_bar = g(np.arange(1, K) / K)
        Y_bar_matrix = np.zeros((num_k, num_tickers))
        for i in range(num_tickers):
            Y_bar_matrix[:, i] = np.convolve(Y_day_np[1:, i], weights_y_bar[::-1], mode='valid')

        # --- 2. 절삭 파라미터(u) 계산 ---
        alpha_u, c0 = 0.235, 4.0
        #std_devs = np.std(m**(1/4) * Y_bar_matrix, axis=0, ddof=1)
        std_devs = np.sqrt(np.sum(np.square(m**(1/4) * Y_bar_matrix))/371)
        u_thresholds = (c0 * std_devs) * (m**(-alpha_u))

        # --- 3. Y_hat 텐서 고속 계산 (np.convolve 사용) ---
        weights_y_hat_sq = (g(np.arange(1, K + 1) / K) - g(np.arange(0, K) / K))**2
        Y_hat_tensor = np.zeros((num_k, num_tickers, num_tickers))
        for i in range(num_tickers):
            for j in range(i, num_tickers):
                prod_returns = Y_day_np[:, i] * Y_day_np[:, j]
                y_hat_series = np.convolve(prod_returns, weights_y_hat_sq[::-1], mode='valid')
                Y_hat_tensor[:, i, j] = y_hat_series
                Y_hat_tensor[:, j, i] = y_hat_series
...
if __name__ == '__main__':
    ...
    try:
        ...
        prvm_results, tickers_list = calculate_prvm(cleaned_data_full_days, start_year=2016, end_year=2019)

        if prvm_results:
            plot_log_kurtosis(prvm_results, tickers_list)
            save_as_single_csv(prvm_results, tickers_list, "prvm_0731.csv")
        else:
