...
def fit_idio_component(args: Tuple) -> Tuple[int, HuberRegressor, float]:
    ...
    if model_type in ['h-lasso', 'lasso']:
        for c_eta in c_eta_candidates:
            ...
            if model_type == 'h-lasso':
                reg = HuberRegressor(fit_intercept=False, alpha=eta_I, epsilon=tau_I)
            else:
    ...
    if model_type == 'h-lasso':
        final_regressor = HuberRegressor(fit_intercept=False, alpha=final_eta_I, epsilon=tau_I)
...
class FIVARModel:
    ...
    def fit(self, prvm_matrices: Dict, idio_matrices: Dict, tickers: List[str]):
        ...
        # [수정] OLS MSPE 값 복원을 위해 PSD 변환을 제거 (이전 버전으로 롤백)
        ...
        X_factor = X_df.iloc[:, :self.r * self.h]
        for i in range(self.r):
            if self.model_type == 'h-lasso':
                self.factor_regressors[i] = HuberRegressor(fit_intercept=False, alpha=0.0, epsilon=tau_F)
            else:
                self.factor_regressors[i] = LinearRegression(fit_intercept=False)
            
            with warnings.catch_warnings():
                warnings.filterwarnings("ignore", category=ConvergenceWarning)
                self.factor_regressors[i].fit(X_factor, y_df.iloc[:, i])
        ...
        if self.model_type == 'ols':
            ...
            tasks = [(i, y_df.iloc[:, self.r + i].copy(), X_df.copy(), self.p, n_reg, c_eta_candidates, self.model_type, tau_I) for i in range(self.p)]
...
if 'prvm_data_all' in locals():
    for t in tqdm(range(start_idx, len(dates)), desc="POET-PRVM 예측"):
        ...
        in_sample_prvm = prvm_data_all[train_start_idx:t]
...
if 'forecasts' in locals() and forecasts['H-LASSO']:
    try:
        ...
        for i, period_name in enumerate(periods):
            ax = axes[i]
            for model in models:
                ax.plot(exposure_constraints, portfolio_risks[model][period_name], marker='o', markersize=4, linestyle='--', label=model)
