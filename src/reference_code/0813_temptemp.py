...
# --- [추가] scikit-learn 공식 코드를 기반으로 epsilon 제약이 없는 Custom Huber Regressor 구현 ---
...
def truncate_data(data: np.ndarray, omega: float) -> np.ndarray:
    ...
    논문의 psi_omega(x) 함수에 해당합니다.
    """
    return np.clip(data, -omega, omega)

# --- [추가] 점프 조정 없는 전체 PRVM 계산 함수 ---
def calculate_prvm_total_from_log_prices(daily_log_prices: np.ndarray, K: int) -> np.ndarray:
    """
    하루치 고빈도 로그 가격 데이터로부터 점프 조정이 없는 전체 PRVM을 계산합니다.
    논문 부록 (A.2) 수식에서 점프 절단(truncation) 부분을 제외하고 구현합니다.
    """
    ...
    # 노이즈 분산 추정
    noise_variance_term = np.zeros((p, p))
    for l in range(1, K):
        g_diff_sq = (min(l, K-l)/K - min(l-1, K-(l-1))/K)**2
        term = daily_returns[:, l-1:m-K+l] * daily_returns[:, l-1:m-K+l]
        noise_variance_term += g_diff_sq * np.sum(term, axis=1)[:, np.newaxis]

    # PRVM 계산 (점프 절단 없음)
    prvm_matrix = (1 / (psi * K)) * (pre_averaged_returns @ pre_averaged_returns.T)
...
class FIVARModel:
    ...
    def fit(self, prvm_matrices: Dict, idio_matrices: Dict, tickers: List[str]):
        ...
        X_df = eigenvalue_df.iloc[:-self.h] 
        ...
        self.omega_F = c_F1 * sigma_F * (n_reg / np.log(self.p))**0.25
        self.omega_I = c_I1 * (n_reg / np.log(self.p))**0.25

        X_truncated = X_df.copy()
        factor_cols = X_truncated.columns[:self.r]
        idio_cols = X_truncated.columns[self.r:]
        ...
        X_scaled = self.scaler_X.fit_transform(X_truncated)
    ...
    def predict(self) -> np.ndarray:
        ...
        X_pred_truncated = X_pred_raw.copy()
        X_pred_truncated[:, :self.r] = truncate_data(X_pred_truncated[:, :self.r], self.omega_F)
        X_pred_truncated[:, self.r:] = truncate_data(X_pred_truncated[:, self.r:], self.omega_I)
...
if prvm_data_all is not None:
    ...
    # --- [수정] DataY.csv에서 직접 점프 변동성 계산 ---
    try:
        print(f"'{log_price_filepath}' 파일에서 로그 가격 데이터를 불러와 점프 변동성을 계산합니다...")
        log_price_df = pd.read_csv(log_price_filepath, header=0, index_col=0, encoding='latin1', low_memory=False)
        log_price_df.index = pd.to_datetime(log_price_df.index)
        
        # 날짜별로 그룹화하여 처리 준비
        daily_groups = log_price_df.groupby(log_price_df.index.date)
        
        jump_vol_all = np.zeros_like(prvm_data_all)
        
        # 논문과 동일한 K값 계산 (1분 데이터 기준 m=390)
        m = 390 
        K = int(np.floor(m**0.5))

        for t, date in enumerate(tqdm(dates, desc="점프 변동성 계산 중")):
            day_str = date.strftime('%Y-%m-%d')
            if date.date() in daily_groups.groups:
                daily_log_prices = daily_groups.get_group(date.date()).T.values
                
                # 점프 조정 없는 전체 PRVM 계산
                prvm_total = calculate_prvm_total_from_log_prices(daily_log_prices, K)
                
                # 점프 변동성 = 전체 변동성 - 연속 변동성
                jump_vol = prvm_total - prvm_data_all[t]
                jump_vol[jump_vol < 0] = 0
                jump_vol_all[t] = jump_vol
        
        print("점프 변동성 행렬 계산 완료.")

    except FileNotFoundError:
        print(f"\n경고: '{log_price_filepath}' 파일을 찾을 수 없습니다.")
        print("점프 변동성 없이 포트폴리오 분석을 진행합니다. 논문과 결과가 다를 수 있습니다.")
        jump_vol_all = np.zeros_like(prvm_data_all)
    except Exception as e:
        print(f"\n점프 변동성 계산 중 오류 발생: {e}")
        jump_vol_all = np.zeros_like(prvm_data_all)
...
if 'forecasts' in locals() and forecasts['H-LASSO']:
    try:
        ...
        for period_name, (start_date, end_date) in periods.items():
            ...
            for model in models:
                ...
                for c0 in tqdm(exposure_constraints, leave=False, desc=f"Exposure {model}"):
                    ...
                    for i, day_idx in enumerate(period_forecast_indices):
                        # --- [수정] 점프 변동성을 포함한 전체 리스크 행렬 계산 ---
                        continuous_vol_forecast = forecasts[model][day_idx]
                        
                        # 예측일(t)의 전날(t-1)의 점프 변동성을 예측치로 사용
                        # forecast_dates의 day_idx에 해당하는 날짜의 전체 데이터 인덱스를 찾아 -1
                        current_date = forecast_dates[day_idx]
                        previous_day_index = dates.get_loc(current_date) - 1
                        
                        if previous_day_index < 0:
                            # 첫날 예측이라 전날 데이터가 없는 경우 점프를 0으로 간주
                            jump_vol_forecast = np.zeros_like(continuous_vol_forecast)
                        else:
                            jump_vol_forecast = jump_vol_all[previous_day_index]

                        # 최종 리스크 행렬 = 연속 변동성 예측치 + 점프 변동성 예측치
                        total_vol_forecast = continuous_vol_forecast + jump_vol_forecast
                        # --- [수정 끝] ---
                        
                        n_assets = total_vol_forecast.shape[0]
                        
                        w = cp.Variable(n_assets)
                        prob = cp.Problem(cp.Minimize(cp.quad_form(w, total_vol_forecast)),
        ...
        style_map = {
            ...
            'LASSO':     {'color': 'green', 'marker': 'o', 'linestyle': '--', 'mfc': 'none'}, 
