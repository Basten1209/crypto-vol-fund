# -*- coding: utf-8 -*-
"""
FIVAR Model Implementation and Evaluation (Optimization Tuned)
...
주요 수정 사항 (v11):
- [핵심 수정] H-LASSO와 LASSO 모델의 수렴성을 높이기 위해 최적화 알고리즘의
  최대 반복 횟수(max_iter)와 허용 오차(tol)를 상향 조정했습니다.
  이를 통해 ConvergenceWarning을 줄이고 H-LASSO 모델의 성능을 개선합니다.
"""
...
class CustomHuberRegressor(BaseEstimator, RegressorMixin):
    ...
    # [수정] H-LASSO의 max_iter를 2000으로, tol을 1e-5로 조정
    def __init__(self, alpha: float = 0.0, epsilon: float = 1.35, max_iter: int = 2000, tol: float = 1e-5):
        self.alpha = alpha
    ...
    def _fit_idio_models(self, X_full_scaled, y_scaled, idio_evals):
        ...
        def find_best_lasso(i):
            ...
            for c_eta in np.linspace(0.1, 10.0, 20):
                # [수정] LASSO의 max_iter를 2000으로, tol을 1e-4로 조정
                eta_I = c_eta * np.sqrt(np.log(self.p) / n_reg); reg = Lasso(alpha=eta_I, fit_intercept=False, max_iter=2000, tol=1e-4).fit(X_full_scaled, y_i)
